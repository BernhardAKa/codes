...
$( document ).ready(function() {
    ...
    $( this ).before('<div style="text-align: right;"><button  class="undo btn  btn-warning"  disabled  type="button">Rückgängig</button><button class="reset btn  btn-danger" disabled  type="button">Reset</button><button class="normalize btn btn-primary" style="color: #fff;"  type="button">Normalisieren</button></div>');
    $( this ).after('<div class="mt-2" style="text-align: center;margin-left:50px"><a class="SubmitDistribution btn btn-success">Verteilung bestätigen</a></div>');
    });
    ...
    function chart(chartindex, params){
        ...
        max_tick = min_tick + (nb_bins - 1) * step_tick;
        model_data = [1, 0];
                        ...
                        events: {
                            ...
                            click: function(e) {
                                ...
                                if (this.series.index == 0) {
                                    if (this.index == 0) { // si point extrÃ¨me
                                        this.update([min, 0]);
                                        update_bins(this);
